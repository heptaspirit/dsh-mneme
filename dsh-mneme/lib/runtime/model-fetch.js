 ...
 * @param {Function} [opts.onEvent] - resume / progress / restart / fail / passthrough 事件回调。
...
async function downloadWithResume(url, init, ctx) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    // 调用方已取消：已 abort 的 signal 不会再发 abort 事件，不在这里查的话
    // 重试循环会照常跑完（退避 + 全量下载），下出一份没人要的文件。
    if (init?.signal?.aborted) {
      throw init?.signal?.reason ?? new Error("下载已被调用方取消");
    }
    let offset = 0;
    ...
    try {
      ...
      if (res.status >= 400 && res.status < 500) {
        // 4xx（可选模型探测 404 是常态）是请求本身的错，重试与退避救不了；
        // 原样透传给 hub 走既有非 200 失败路径，锁在返回前释放。
        releaseLock(lockPath, lockFd);
        return res;
      }
      // 5xx 是服务端瞬态故障，值得重试。
      ...
      // 压缩响应（CDN 对 tokenizer/config 等小文件常开 gzip/br）：res.body 是解码后的
      // 流而 content-length 是编码字节数，完整性基准不成立；用解码偏移发 Range 还会
      // 拼坏文件。不落盘、不续传：清掉旧断点，这份解码流原样透传（下游消费完即弃）。
      const encoding = res.headers.get("content-encoding");
      if (encoding && encoding.trim().toLowerCase() !== "identity") {
        emit({ type: "passthrough", url, attempt, reason: encoding.trim().toLowerCase() });
        resetPart(partPath, metaPath);
        releaseLock(lockPath, lockFd);
        return res;
      }
...
export function logModelFetchEvent(logger, event) {
  ...
  switch (event.type) {
    ...
    case "fail":
      ...
      break;
    case "passthrough":
      logger?.info?.(`[dsh-mneme] 模型下载响应为压缩传输（${event.reason}），本次不经断点续传`);
      break;
    default:
      break;
  }
