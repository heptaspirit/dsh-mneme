...
test("model-fetch：调用方已取消的 signal 直接抛出，一次请求都不发", async () => {
  const srv = await serve({ "/model.onnx": makeBody(1000) });
  const dir = partDirFor();
  try {
    let calls = 0;
    const fetchImpl = testFetcher(dir, { fetchImpl: (...a) => { calls++; return fetch(...a); } });
    const ac = new AbortController();
    ac.abort(new Error("no longer needed")); // 预先取消：这种 signal 不会再发 abort 事件。
    await assert.rejects(
      () => fetchImpl(`${srv.base}/model.onnx`, { signal: ac.signal }),
      /no longer needed/,
      "抛出的应是调用方的 reason，让上游能辨认取消"
    );
    assert.equal(calls, 0, "已取消就不该再发任何请求");
    assert.deepEqual(readdirSync(dir), [], "不该留下断点文件或锁");
  } finally {
    await srv.close();
  }
});

test("model-fetch：4xx 原样透传——不烧重试与退避，锁不残留", async () => {
  const srv = await serve({}); // 任何 URL 都 404（可选模型探测的常态）
  const dir = partDirFor();
  try {
    let calls = 0;
    const fetchImpl = testFetcher(dir, { fetchImpl: (...a) => { calls++; return fetch(...a); } });
    const res = await fetchImpl(`${srv.base}/missing-model.bin`);
    assert.equal(res.status, 404, "4xx 原样透传，hub 按既有非 200 路径处理");
    assert.equal(calls, 1, "4xx 不该消耗重试次数");
    assert.deepEqual(readdirSync(dir), [], "透传不产生断点文件，锁已释放");
  } finally {
    await srv.close();
  }
});

test("model-fetch：压缩响应透传解码流——不落盘、不续传、字节原样", async () => {
  const body = makeBody(1000);
  const wire = gzipSync(body); // 线上字节真的压缩；content-length 是编码后长度。
  const server = createServer((_req, res) => {
    res.writeHead(200, { "Content-Length": String(wire.length), "Content-Encoding": "gzip", ETag: '"gz"' });
    res.end(wire);
  });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  const base = `http://127.0.0.1:${server.address().port}`;
  ...
  try {
    const events = [];
    const fetchImpl = testFetcher(dir, { onEvent: (e) => events.push(e) });
    const res = await fetchImpl(`${base}/config.json`);
    assert.equal(res.status, 200);
    assert.ok(Buffer.from(await res.arrayBuffer()).equals(body), "解码流应原样到达下游");
    assert.ok(events.some((e) => e.type === "passthrough" && e.reason === "gzip"), "应有 passthrough 事件说明跳过续传");
    assert.equal(readdirSync(dir).length, 0, "压缩响应不落盘");
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
});
